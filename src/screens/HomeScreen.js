...
export default function HomeScreen({ navigation }) {
  ...
  const suggestions = useMemo(() => {
    ...
    lists.forEach((list) => {
      (list.items || []).forEach((item) => {
        ...
        map[key].count++;
        map[key].price = item.price; // use most recent price
      });
    });
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return suggestions
      .filter((s) => s.name.toLowerCase().includes(itemName.toLowerCase()))
      .slice(0, 5);
  ...
  const addItem = useCallback(() => {
    ...
    if (!name) {
      ...
      showToast('Въведете наименование на продукта', 'warning');
      return;
    }
    if (!price || price <= 0) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
      showToast('Въведете валидна цена', 'warning');
      return;
    }
  ...
  const handleStartShopping = () => {
    ...
    navigation.navigate('ShoppingList', {
      list: items,
      budget: budgetNum,
      listName: listName.trim() || 'Моят списък',
      store,
    });
  };
  ...
  const handleSaveTemplate = () => {
    ...
    Alert.prompt(
      'Запази като шаблон',
      'Въведете наименование на шаблона:',
      async (name) => {
        if (!name?.trim()) return;
  ...
  const handleAddStore = async () => {
    ...
    if (ok) {
      setNewStoreName('');
      showToast('Магазинът е добавен', 'success');
    } else {
      showToast('Магазинът вече съществува', 'warning');
    }
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.section}>
          <View style={styles.sectionRow}>
            ...
            <TouchableOpacity onPress={() => setStoreModalVisible(true)}>
              <Text style={styles.sectionLink}>+ Добави магазин</Text>
            </TouchableOpacity>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.storeRow}>
            {stores.map((s) => (
              <TouchableOpacity
                ...
                onLongPress={() => {
                  if (!customs.includes(s)) return;
                  Alert.alert('Премахни магазин', `Премахни "${s}"?`, [
                    { text: 'Отказ', style: 'cancel' },
                    { text: 'Премахни', style: 'destructive', onPress: () => { removeStore(s); if (store === s) setStore('Всички'); } },
                  ]);
                }}
                activeOpacity={0.8}
              >
          ...
          <View style={styles.addCard}>
            ...
            <View style={styles.addRow}>
              <View style={[styles.inputCard, { flex: 1, marginRight: 8 }]}>
                ...
                <TextInput
                  ...
                  placeholderTextColor="#bbb"
                  value={itemName}
                  onChangeText={(v) => { setItemName(v); setShowSuggestions(true); }}
              ...
              <View style={[styles.inputCard, { width: 110 }]}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.catRow}>
              {CATEGORIES.map((cat) => (
                <TouchableOpacity
                  key={cat.id}
                  style={[styles.catChip, itemCategory === cat.id && styles.catChipActive]}
                  onPress={() => { Haptics.selectionAsync(); setItemCategory(cat.id); }}
                  activeOpacity={0.8}
                >
                  <Text style={styles.catEmoji}>{cat.emoji}</Text>
                  <Text style={[styles.catLabel, itemCategory === cat.id && styles.catLabelActive]}>
                    {cat.label}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <View style={styles.addFooter}>
              ...
              <TouchableOpacity
                ...
                onPress={() => { Haptics.selectionAsync(); setShowNote((v) => !v); }}
              >
            ...
            {showNote && (
              <View style={styles.noteInputWrap}>
                ...
                <TextInput
                  ...
                  placeholder="напр. само ако е намалено"
        ...
        <View style={styles.actionRow}>
          <AnimatedPressable
            style={[styles.shopBtn, items.length === 0 && styles.btnDisabled]}
      ...
      <Modal visible={storeModalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <Text style={styles.modalSub}>Задръжте върху персонализиран магазин за да го премахнете</Text>

            <View style={styles.storeAddRow}>
              <TextInput
                style={styles.storeAddInput}
                ...
                >
                  <View style={styles.tplCardHeader}>
                    ...
                    {tpl.store && tpl.store !== 'Всички' && (
                      <View style={styles.tplStoreBadge}>
                        <Text style={styles.tplStoreBadgeText}>{tpl.store}</Text>
                      </View>
                    )}
                  </View>
...
const styles = StyleSheet.create({
  ...
  logoutBtn: {
    ...
    backgroundColor: '#eee', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, marginTop: 4,
  },
  ...
  tplChip: {
    ...
    borderWidth: 1, borderColor: '#E0DCFF',
  },
  ...
  inputCard: {
    ...
    borderRadius: 14, paddingHorizontal: 14, paddingVertical: 4, gap: 10,
  ...
  storeRow: { gap: 10, paddingRight: 4 },
  storeChip: {
    paddingHorizontal: 18, paddingVertical: 10, borderRadius: 24, backgroundColor: '#fff',
  ...
  suggestionsBox: {
    backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#F0EEFF',
    overflow: 'hidden',
  },
  suggestionRow: {
    flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10,
    borderBottomWidth: 1, borderBottomColor: '#F7F8FC', gap: 8,
  },
  ...
  suggestionCount: { fontSize: 11, color: '#aaa' },
  ...
  addBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    backgroundColor: '#6C63FF', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12,
  },
  addBtnText: { color: '#fff', fontWeight: '700', fontSize: 14 },
  noteInputWrap: {
    flexDirection: 'row', alignItems: 'flex-start', gap: 8,
    backgroundColor: '#F7F8FC', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8,
  },
  ...
  summaryCard: {
    backgroundColor: '#fff', borderRadius: 18, padding: 18, marginBottom: 22,
  ...
  shopBtn: {
    ...
    backgroundColor: '#F0EEFF', borderRadius: 18, paddingVertical: 18,
    borderWidth: 2, borderColor: '#6C63FF',
  },
  shopBtnText: { color: '#6C63FF', fontWeight: '800', fontSize: 16 },
  saveBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
    backgroundColor: '#6C63FF', borderRadius: 18, paddingVertical: 18,
    shadowColor: '#6C63FF', shadowOpacity: 0.4, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 6,
  },
  ...
  storeAddBtn: {
    backgroundColor: '#6C63FF', borderRadius: 12, width: 46, height: 46,
    justifyContent: 'center', alignItems: 'center',
  },
  storeManageRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#F7F8FC',
  },
  storeManageName: { fontSize: 15, fontWeight: '600', color: '#1A1A2E' },
  modalClose: {
    backgroundColor: '#F7F8FC', borderRadius: 14, paddingVertical: 14, alignItems: 'center',
  },
  modalCloseText: { fontSize: 15, fontWeight: '700', color: '#555' },

  tplCard: {
    backgroundColor: '#F7F8FC', borderRadius: 14, padding: 14,
    borderWidth: 1.5, borderColor: '#eee', gap: 6,
  },
