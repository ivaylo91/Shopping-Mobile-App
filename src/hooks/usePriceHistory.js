...
export function usePriceHistory() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEYS.PRICE_HISTORY).then((raw) => {
  ...
  const persist = async (data) => {
    setHistory(data);
    await AsyncStorage.setItem(STORAGE_KEYS.PRICE_HISTORY, JSON.stringify(data));
  };
  ...
  const recordPrices = async (items, store) => {
    ...
    for (const item of items) {
      ...
      next[key] = [{ price: item.price, store, date: now }, ...existing].slice(0, STORAGE_LIMITS.MAX_PRICE_HISTORY_PER_PRODUCT);
    }
